...
# 🎨 Fonts
HEADING_FONT = ("Cambria", 20, "bold")   # Titles / Headings
SUBHEADING_FONT = ("Cambria", 14, "bold")
BODY_FONT = ("Georgia", 12)                       # Normal text / input / results
BODY_FONT_BOLD = ("Georgia", 12, "bold")

class IEEE754GUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("IEEE-754 Calculator (32-bit) ✨")
        self.configure(bg=COLORS["almond"])
        self.geometry("1000x600")

        # Title Bar
        title = tk.Label(self, text="IEEE-754 Calculator (32-bit) 🧮",
    ...
    def build_left_panel(self):
        # Decimal Input
        tk.Label(self.left_frame, text="🧮 Decimal Input:",
        ...
        # IEEE Input
        tk.Label(self.left_frame, text="💻 IEEE 754 (32-bit Input):",
                 font=SUBHEADING_FONT,
                 bg=COLORS["rose"], fg=COLORS["black"]).pack(anchor="w", padx=5, pady=5)

        self.ieee_entry = tk.Entry(self.left_frame, width=35, font=BODY_FONT)
    ...
    def build_right_panel(self):
        # Results Section
        tk.Label(self.right_frame, text="📊 Results",
                 font=SUBHEADING_FONT,
                 bg=COLORS["rose"], fg=COLORS["black"]).pack(anchor="w", padx=5, pady=5)

        self.result_box = tk.Text(self.right_frame, height=5, bg="white", fg="black", wrap="word", font=BODY_FONT)
        self.result_box.pack(fill="x", padx=5, pady=5)

        # Output Breakdown
        tk.Label(self.right_frame, text="🔎 Output Breakdown",
                 font=SUBHEADING_FONT,
                 bg=COLORS["rose"], fg=COLORS["black"]).pack(anchor="w", padx=5, pady=5)

        self.breakdown_box = tk.Text(self.right_frame, height=3, bg="white", fg="black", wrap="word", font=BODY_FONT)
        self.breakdown_box.pack(fill="x", padx=5, pady=5)

        # Learning Mode
        self.learning_btn = tk.Button(self.right_frame,
                                      text="📘 Learning Mode — Step by Step",
                                      command=self.show_learning_mode,
                                      bg=COLORS["violet"], fg="white", font=BODY_FONT_BOLD)
        self.learning_btn.pack(fill="x", padx=5, pady=5)

        self.learning_text = tk.Text(self.right_frame, height=12, bg="white", fg="black", wrap="word", font=BODY_FONT)
